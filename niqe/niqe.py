...
def get_niqe(img:np.ndarray) -> float:
    ''' standard RGB image in uint8 '''
    assert len(img.shape) == 3 and img.shape[-1] == 3, 'shape must be [H, W, C]'
    assert img.dtype == np.uint8, 'dtype must be np.uint8'
    assert 0 <= img.min() and img.max() <= 255, 'vrng must in [0, 255]'
    return calculate_niqe(img[:, :, ::-1])      # RGB2BGR
